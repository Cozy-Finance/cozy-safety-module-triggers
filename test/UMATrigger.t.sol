...
import "uma-protocol/packages/core/contracts/data-verification-mechanism/interfaces/OracleAncillaryInterface.sol";
import "../src/UMATriggerFactory.sol";
import "./utils/TriggerTestSetup.sol";
...
contract DeployTriggerSharedTest is TriggerTestSetup {
  ...
  function testFork_DeployTriggerDeploysANewTrigger(
    ...
    address _computedTriggerAddress = factory.computeTriggerAddress(
      "Has Terra been hacked?", rewardToken, _rewardAmount, refundRecipient, _bondAmount, _proposalDisputeWindow
    );
  ...
  function _getDVM() internal view returns (OracleAncillaryInterface) {
    return OracleAncillaryInterface(FinderInterface(umaOracleFinder).getImplementationAddress(bytes32("Oracle")));
  }

  function _settleQueryViaDVM(int256 _answer, address _requester, uint256 _queryTimestamp, bytes memory _query) public {
    ...
    vm.mockCall(
      address(_getDVM()), abi.encodeWithSelector(OracleAncillaryInterface.hasPrice.selector), abi.encode(true)
    );
    vm.mockCall(
      address(_getDVM()), abi.encodeWithSelector(OracleAncillaryInterface.getPrice.selector), abi.encode(_answer)
    );
    umaOracle.settle(_requester, queryIdentifier, _queryTimestamp, _query);
    vm.clearMockedCalls();
  }
  ...
  function testFork_RunProgrammaticCheckSettlesRequests(
    ...
    bool _isDisputed,
    string memory _queryDifferentiator // You can't create the exact same UMA trigger more than once.
  ) internal {
    ...
    _vars.query = string(abi.encodePacked("q: Has protocol XYZ been hacked?", _queryDifferentiator));
    ...
    _vars.umaRequest =
      umaOracle.getRequest(address(_vars.trigger), queryIdentifier, _vars.queryTimestamp, bytes(_vars.query));
    ...
    if (_settledAnswer != AFFIRMATIVE_ANSWER) {
      ...
      if (_settledAnswer == TOO_EARLY_ANSWER) vm.expectRevert("Cannot propose 'too early'");
      else vm.expectRevert(UMATrigger.InvalidProposal.selector);
      umaOracle.proposePrice(
        address(_vars.trigger), queryIdentifier, _vars.queryTimestamp, bytes(_vars.query), _settledAnswer
      );
    ...
    vm.prank(_vars.proposer);
    umaOracle.proposePrice(
      address(_vars.trigger), queryIdentifier, _vars.queryTimestamp, bytes(_vars.query), AFFIRMATIVE_ANSWER
    );
    assertEq(_vars.trigger.state(), TriggerState.FROZEN);
    ...
    if (_isDisputed) {
      ...
      umaOracle.disputePrice(address(_vars.trigger), queryIdentifier, _vars.queryTimestamp, bytes(_vars.query));
      vm.stopPrank();
    ...
    if (_isExternallySettled) {
      ...
      vm.prank(_vars.settler);
      umaOracle.settle(address(_vars.trigger), queryIdentifier, _vars.queryTimestamp, bytes(_vars.query));
    ...
    if (_settledAnswer != AFFIRMATIVE_ANSWER && _isExternallySettled) vm.expectRevert(UMATrigger.Unsettleable.selector);
    vm.prank(_vars.settler);
    _vars.trigger.runProgrammaticCheck();

    if (_settledAnswer == AFFIRMATIVE_ANSWER) {
      assertEq(_vars.trigger.state(), TriggerState.TRIGGERED);
      if (_isDisputed) {
        if (_isExternallySettled) {
          // The reward was sent back to the trigger creator.
          assertEq(rewardToken.balanceOf(_vars.deployer), _vars.initDeployerBalance + _vars.rewardAmount);
        } else {
          // The reward was sent to the settler.
          assertEq(rewardToken.balanceOf(_vars.settler), _vars.initSettlerBalance + _vars.rewardAmount);
        }
      } else {
  ...
  function testFork_DeployTriggerCreatesAUMARequest(
    ...
    _umaRequest =
      umaOracle.getRequest(address(_trigger), queryIdentifier, _queryTimestamp, bytes("Has Terra been hacked?"));
    ...
    rewardToken.approve(address(umaOracle), _bondAmount + _umaRequest.finalFee);
    umaOracle.disputePrice(address(_trigger), queryIdentifier, _queryTimestamp, bytes("Has Terra been hacked?"));
    vm.stopPrank();

    // Settle and have the DVM side with the disputer: there was no hack.
    _settleQueryViaDVM(NEGATIVE_ANSWER, address(_trigger), _queryTimestamp, bytes("Has Terra been hacked?"));
    ...
    // Settle the request.
    assertEq(_umaRequest.settled, false);
    umaOracle.settle(address(_trigger), queryIdentifier, _queryTimestamp, bytes("Has Terra been hacked?"));
    _umaRequest =
      umaOracle.getRequest(address(_trigger), queryIdentifier, _queryTimestamp, bytes("Has Terra been hacked?"));
    assertEq(_umaRequest.settled, true);

    // Run the trigger programmatic check.
    assertEq(_trigger.runProgrammaticCheck(), TriggerState.TRIGGERED);
  }
  ...
  function testFork_TriggerFreezesMarketsWhenAnswersAreProposed(
    ...
    OptimisticOracleV2Interface.Request memory _umaRequest;
    _umaRequest =
      umaOracle.getRequest(address(_trigger), queryIdentifier, _queryTimestamp, bytes("Has USDT been hacked?"));
    ...
    assertEq(_trigger.state(), TriggerState.FROZEN);

    // Have someone else dispute the answer.
    deal(address(rewardToken), address(42), _bondAmount + _umaRequest.finalFee);
    vm.startPrank(address(42));
    rewardToken.approve(address(umaOracle), _bondAmount + _umaRequest.finalFee);
    umaOracle.disputePrice(address(_trigger), queryIdentifier, _queryTimestamp, bytes("Has USDT been hacked?"));
    vm.stopPrank();

    // Settle and have the DVM side with the disputer: there was no hack.
    _settleQueryViaDVM(NEGATIVE_ANSWER, address(_trigger), _queryTimestamp, bytes("Has USDT been hacked?"));
...
contract DeployTriggerMainnetTest is DeployTriggerSharedTest {
  function setUp() public override {
    ...
    uint256 mainnetForkBlock = 15_238_191; // The mainnet block number at the time this test was written.
    ...
    factory = new UMATriggerFactory(umaOracle);
  }
  ...
  function testFork1_RunProgrammaticCheckSettlesRequests() public {
    //                                            oracle answer,   externally settled,  disputed, queryDifferentiator
    testFork_RunProgrammaticCheckSettlesRequests(AFFIRMATIVE_ANSWER, false, false, "0");
    testFork_RunProgrammaticCheckSettlesRequests(AFFIRMATIVE_ANSWER, true, false, "1");
    testFork_RunProgrammaticCheckSettlesRequests(AFFIRMATIVE_ANSWER, false, true, "2");
    testFork_RunProgrammaticCheckSettlesRequests(AFFIRMATIVE_ANSWER, true, true, "3");
    testFork_RunProgrammaticCheckSettlesRequests(NEGATIVE_ANSWER, false, true, "4");
    testFork_RunProgrammaticCheckSettlesRequests(NEGATIVE_ANSWER, true, true, "5");
    testFork_RunProgrammaticCheckSettlesRequests(INDETERMINATE_ANSWER, false, true, "6");
...
contract DeployTriggerOptimismTest is DeployTriggerSharedTest {
  function setUp() public override {
    ...
    uint256 optimismForkBlock = 25_582_446;
